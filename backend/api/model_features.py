...
def is_high_bp(blood_pressure):
    if blood_pressure is None:
        return False  # Default value when blood pressure is not recorded
    return blood_pressure > 130  # Assuming 130 is your threshold for high blood pressure
...
def is_high_cholesterol(user):
    ...
    if meals.exists():
        # Add null check for cholesterol values
        total_cholesterol = sum(meal.cholesterol or 0 for meal in meals)
...
def calculate_mental_health(user, start_date):
    ...
    if stress_records.exists():
        # Add null check for stress_level values
        total_stress_level = sum(record.stress_level or 0 for record in stress_records if record.stress_level is not None)
        if not any(record.stress_level is not None for record in stress_records):
            return 0  # No valid stress records
        valid_records = sum(1 for record in stress_records if record.stress_level is not None)
        average_stress_level = total_stress_level / valid_records
        return 1 if average_stress_level >= 3 else 0
    return 0
...
def calculate_physical_health(user, start_date):
    ...
    if phys_activity_records.exists():
        # Add null check for duration values
        total_duration = sum(record.duration or 0 for record in phys_activity_records)
