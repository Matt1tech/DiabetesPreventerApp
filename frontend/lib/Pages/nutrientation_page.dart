...
class NutrientationPage extends StatefulWidget {
  ...
  const NutrientationPage({super.key, required this.imageFile});
...
class _NutrientationPageState extends State<NutrientationPage> {
  ...
  Future<void> _analyzeImage() async {
    ...
    try {
      final data = await analyzeImage(widget.imageFile);
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
              content: Text('Food analysis is temporarily unavailable.')),
        );
      }
    } finally {
      setState(() {
